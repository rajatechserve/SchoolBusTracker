...
export default function Assignments(){
	...
	const load=()=>api.get('/assignments', { params: { search: q || undefined } }).then(r=>setList(r.data||[])).catch(()=>{});
	useEffect(()=>{ load(); },[q]);
	useEffect(()=>{ api.get('/drivers').then(r=>setDrivers(r.data||[])); api.get('/buses').then(r=>setBuses(r.data||[])); api.get('/routes').then(r=>setRoutes(r.data||[])); },[]);
	const save=async()=>{ if(isViewer) return; try{ await api.post('/assignments', form); load(); }catch(e){alert('Error');} };
	const remove=async(id)=>{ if(isViewer) return; if(!confirm('Delete?')) return; await api.delete('/assignments/'+id); load(); };
	return (<div><div className='flex items-center justify-between mb-4'><h2 className='text-xl font-semibold'>Assignments {isViewer && <span className='text-xs text-slate-500'>(read-only)</span>}</h2><input placeholder='Search' value={q} onChange={e=>setQ(e.target.value)} className='border p-2'/></div>
